...
let cx = ctx.canvas.width / 1
let cy = ctx.canvas.height / 1
...
render = () => {
  ...
  confetti.forEach((confetto, index) => {
    // Confetti size
    let mltply = 3.5
    let width = (confetto.dimensions.x * (mltply * confetto.scale.x))
    let height = (confetto.dimensions.y * (mltply * confetto.scale.y))
  ...
  sequins.forEach((sequin, index) => {  
    ...
    // Sequin size v
    ctx.arc(0, 0, (5 * sequin.radius), 0, 2 * Math.PI)
    ctx.fill()
